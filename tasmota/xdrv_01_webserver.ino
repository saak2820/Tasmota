...
const char HTTP_FORM_OTHER[] PROGMEM =
  ...
  "<br>"
  "<label><input id='b4' type='checkbox'%s><b>" D_FRITZBOX_ENABLE "</b></label><br>"
  "<br>"
...
void HandleOtherConfiguration(void)
{
  ...
  WSContentSend_P(HTTP_FORM_OTHER, stemp, (USER_MODULE == Settings.module) ? " checked disabled" : "",
    ...
    (Settings.flag.fritzbox_enabled) ? " checked" : "",   // SetOption32 - Enable FRITZBOX
...
void OtherSaveSettings(void)
{
  ...
  Settings.flag.fritzbox_enabled = Webserver->hasArg("b4");  // SetOption32 - Enable FRITZBOX
#ifdef USE_EMULATION
  UdpDisconnect();
#if defined(USE_EMULATION_WEMO) || defined(USE_EMULATION_HUE)
  ...
  snprintf_P(message, sizeof(message), PSTR(D_LOG_OTHER D_MQTT_ENABLE " %s, " D_CMND_EMULATION " %d, " D_CMND_DEVICENAME " %s, " D_CMND_FRIENDLYNAME),
    GetStateText(Settings.flag.mqtt_enabled),GetStateText(Settings.flag.fritzbox_enabled), Settings.flag2.emulation, SettingsText(SET_DEVICENAME));
...
void HandleInformation(void)
{
  ...
  if (Settings.flag.fritzbox_enabled) {  // SetOption32 - Enable MQTT
    
    AddLog_P(LOG_LEVEL_DEBUG, PSTR(D_LOG_UPLOAD "fritzbox_enabled %d"), Settings.flag.fritzbox_enabled);
    ...
    WSContentSend_P(PSTR("}1" D_FRITZBOX "}2" D_DISABLED));
  }
  WSContentSend_P(PSTR("}1}2&nbsp;"));  // Empty line
#endif // USE_FRITZBOX
